...
async function genImg(os){var c=document.createElement('canvas');var x=c.getContext('2d');var pd=16,lh=22,hh=70;var items=[];var gt=0;os.forEach(function(o){(o.items||[]).forEach(function(it){items.push(it);gt+=it.total||0;});});var ch=items.length*lh+80;c.width=380;c.height=hh+ch+pd*2;x.fillStyle='#f5f5f5';x.fillRect(0,0,c.width,c.height);x.fillStyle='#fff';roundRect(x,pd,pd,c.width-pd*2,c.height-pd*2,10);x.fill();x.fillStyle='#2E7D32';x.font='bold 18px sans-serif';x.textAlign='center';x.fillText(os[0].customer||'',c.width/2,pd+28);x.font='13px sans-serif';x.fillStyle='#666';x.fillText(os[0].date||'',c.width/2,pd+48);x.strokeStyle='#2E7D32';x.beginPath();x.moveTo(pd+12,pd+60);x.lineTo(c.width-pd-12,pd+60);x.stroke();x.fillStyle='#333';x.font='12px sans-serif';x.textAlign='left';x.fillText('品名',pd+20,pd+80);x.textAlign='center';x.fillText('重量',pd+70,pd+80);x.fillText('单价',pd+160,pd+80);x.textAlign='right';x.fillText('金额',c.width-pd-12,pd+80);var cy=pd+98;items.forEach(function(it){x.textAlign='left';x.fillText(it.name||'',pd+20,cy);x.textAlign='center';x.fillText(fmtNum(it.weight)+'斤',pd+70,cy);x.fillText(fmtNum(it.unitPrice)+'元',pd+160,cy);x.textAlign='right';x.fillText(fmtNum(it.total)+'元',c.width-pd-12,cy);cy+=lh;});x.strokeStyle='#ddd';x.beginPath();x.moveTo(pd+12,cy+5);x.lineTo(c.width-pd-12,cy+5);x.stroke();x.fillStyle='#2E7D32';x.font='bold 15px sans-serif';x.textAlign='right';x.fillText('合计：'+fmtNum(gt)+'元',c.width-pd-12,cy+28);x.fillStyle='#bbb';x.font='11px sans-serif';x.textAlign='center';x.fillText('—— 菜农记账 ——',c.width/2,c.height-pd-6);return new Promise(function(r){c.toBlob(function(b){r(b);},'image/png');});}
