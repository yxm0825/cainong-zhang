...
list.querySelectorAll('.month-share-btn').forEach(function(btn){btn.addEventListener('click',async function(e){e.stopPropagation();var c=decodeURIComponent(e.currentTarget.dataset.customer);var m=e.currentTarget.dataset.month;try{var os=await DB.getCustomerMonthOrders(c,d);var blob=await genImg(os);if(await copyImg(blob)){showToast('已复制');}else{var u=URL.createObjectURL(blob);var w=window.open('','_blank');if(w){w.document.write('<img src=\\u0022'+u+'\\u0022 style=\\u0022max-width:100%;margin:0 auto;display:block\\u0022>');}else{var i=document.createElement('img');i.src=u;i.style.cssText='position:fixed;top:0;left:0;width:100%;height:100%;z-index:20000;background:rgba(0,0,0,.8);object-fit:contain;padding:20px;box-sizing:border-box';i.onclick=function(){document.body.removeChild(i)};document.body.appendChild(i)}showToast('长按图片保存，再发送给客户')}}catch(e){showToast('失败')};});});
