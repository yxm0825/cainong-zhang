...
async function genImg(os){var dpr=window.devicePixelRatio||1;var W=380,pd=16,lh=26,cl1=28,cl2=170,cl3=240,cl4=W-pd-12;var items=[];var gt=0;os.forEach(function(o){(o.items||[]).forEach(function(it){items.push(it);gt+=it.total||0;});});var hh=75;var ch=30+items.length*lh+30;var H=hh+ch+pd*2;var ca=document.createElement('canvas');ca.width=W*dpr;ca.height=H*dpr;ca.style.width=W+'px';ca.style.height=H+'px';var x=ca.getContext('2d');x.scale(dpr,dpr);x.fillStyle='#f5f5f5';x.fillRect(0,0,W,H);x.fillStyle='#fff';roundRect(x,pd,pd,W-pd*2,H-pd*2,10);x.fill();x.fillStyle='#2E7D32';x.font='bold 17px sans-serif';x.textAlign='center';x.fillText(os[0].customer||'',W/2,pd+26);x.font='13px sans-serif';x.fillStyle='#666';x.fillText(os[0].date||'',W/2,pd+46);x.strokeStyle='#2E7D32';x.beginPath();x.moveTo(pd+12,pd+56);x.lineTo(W-pd-12,pd+56);x.stroke();x.fillStyle='#333';x.font='12px sans-serif';x.textAlign='left';x.fillText('品名',cl1,pd+74);x.textAlign='center';x.fillText('重量',cl2,pd+74);x.fillText('单价',cl3,pd+74);x.textAlign='right';x.fillText('金额',cl4,pd+74);var cy=pd+92;x.font='14px sans-serif';items.forEach(function(it){x.textAlign='left';x.fillText(it.name||'',cl1,cy);x.textAlign='center';x.fillText(fmtNum(it.weight)+'\u65a4',cl2,cy);x.fillText(fmtNum(it.unitPrice)+'\u5143',cl3,cy);x.textAlign='right';x.fillText(fmtNum(it.total)+'\u5143',cl4,cy);cy+=lh;});x.strokeStyle='#ddd';x.beginPath();x.moveTo(pd+12,cy);x.lineTo(W-pd-12,cy);x.stroke();x.fillStyle='#2E7D32';x.font='bold 15px sans-serif';x.textAlign='right';x.fillText('\u5408\u8ba1\uff1a'+fmtNum(gt)+'\u5143',cl4,cy+24);x.fillStyle='#bbb';x.font='11px sans-serif';x.textAlign='center';x.fillText('\u2500\u2500 \u83dc\u519c\u8bb0\u8d26 \u2500\u2500',W/2,H-pd-6);return new Promise(function(r){ca.toBlob(function(b){r(b);},'image/png');});}
